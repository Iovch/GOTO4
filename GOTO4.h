/*
 * GOTO4_7.h Written by Igor Ovchinnikov 13/10/2016
 */
...
void AscFoSw(void)
{
 ...
 {
  bFocus=true;
  analogWrite(LIHT_FOC_PIN,HIGH); // Светодиод фокусера включен
 }
 else
 {
  bFocus=false;
  analogWrite(LIHT_FOC_PIN,LOW); // Светодиод фокусера выключен
 }
}
...
int AskJOY()
{
  ...
  if(iA1>=25 && iA1 < 480)  { iRetValue=iRetValue |  1; } // Микрошаг X+
  if(iA1>530 && iA1<=1000)  { iRetValue=iRetValue |  4; } // Микрошаг X-
  if(iA1>1000)              { iRetValue=iRetValue | 64; } // Полный шаг X-
  
  if(iA2<25)                { iRetValue=iRetValue | 32; } // Полный шаг Y+
  if(iA2>=25  && iA2 < 380) { iRetValue=iRetValue |  2; } // Микрошаг Y+
  if(iA2>510  && iA2<=950) { iRetValue=iRetValue |  8; } // Микрошаг Y-
  if(iA2>950)              { iRetValue=iRetValue | 128;} // Полный шаг Y-
...
boolean FoStart()
{
  ...
  if (iAF < 275 && FO_START_VAL == LOW ) bRetVal=true;
  if (iAF > 750 && FO_START_VAL == HIGH) bRetVal=true;
  return bRetVal;
}
...
void FoInit(void)
{
  ...
  delay(500);
  Stepper_Z_step(iStDZ*FO_START_STEP);
  ulFomSts=long(iZStepX)*FO_START_STEP;
}
